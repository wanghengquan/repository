...
public class rabbitMQ extends Thread {
	...
	public final static String[] init_key = { "diamond", "diamondng", "modelsim", "questasim", "riviera", "icecube",
			"squish", "radiant" };
	...
	public static int client_publish_send(String content) {
		...
		try {
			...
			Date dt = new Date();
			String localeString = dt.toString();
			file_action.print_out_to_file_to_gui(
					"In Client Publish Send,\n" + localeString + "\nCan not send message(1) to server");
			return 1;
	...
	public static int client_send(String queue_name, String content) {
		 ...
		 * Info if the content send smoothly, 0 will be return Or 1 will be
		 * return.
		 */
		ConnectionFactory factory = new ConnectionFactory();
		factory.setHost(HOST);
	...
	public static String read_task_server(String queue_name) throws Exception {
		...
		// when the client get one message from the server, it should stop the
		// connect!
		ConnectionFactory factory = new ConnectionFactory();
		factory.setHost(HOST);
		factory.setUsername(RABBITMQ_USER);
		...
		if (queue_name.endsWith("queue_30_20_9")) { // this is used for test
													// queue
			v_host = "/";
	...
	public static synchronized void updateAdmin_queueList(String content) {
		/*
		 * This function used to update the admin queue list. and at here, the
		 * client will make check the message is usful or not.
		 */
		...
		if (queue_name == null)
			return;
		// client_task: task queue directly request this client do the task
		// otherwise belong to match task.
		...
		if (request_key.contains("Machine")) {// get client and group from
												// message
		...
		if (request_key.contains("System")) {
			...
			// String monitor_cpu_used = monitor.getCpu_used(); //just process
			// number
			// String monitor_memory_left = monitor.getMemory_left();//just
			// process number
			if (os != null) {
			 ...
			 * if(min_space != null){
			 * if(Float.parseFloat(min_space.replaceAll("[a-zA-Z]", "")) >
			 * Float.parseFloat(monitor_disk_left.replaceAll("[a-zA-Z]", "")))
			 * match_requirement = false; min_space_match = false; } if(min_cpu
			 * != null){ if(Float.parseFloat(min_cpu.replaceAll("[a-zA-Z]", ""))
			 * > Float.parseFloat(monitor_cpu_used.replaceAll("[a-zA-Z]", "")))
			 * match_requirement = false; } if(min_mem != null){
			 * if(Float.parseFloat(min_mem.replaceAll("[a-zA-]", "")) >
			 * Float.parseFloat(monitor_memory_left.replaceAll("[a-zA-]", "")) )
			 * match_requirement = false; }
			 */
		}
			...
			while (iterator_key.hasNext()) {
				...
				if (request_value instanceof String) {
					...
					if (Arrays.asList(init_key).contains(key.trim())) {
						try {
							conf_value = ConfigurationFile.getProfileString(defaultConfigure, key.trim(), request_value,
									"default");
						...
						try {
							conf_value = ConfigurationFile.getProfileString(defaultConfigure, "soft", request_value,
									"default");
						} catch (IOException e) {
							e.printStackTrace();
							Date dt = new Date();
		...
		if (match_requirement) {
			if (action.matches("pause")) {
				...
				if (task_queue_array.toString().indexOf(queue_name) < 0) {
					...
					try {
						client_private = ConfigurationFile.getProfileString(defaultConfigure, "machine", "private",
								null);
					} catch (IOException e) {
						e.printStackTrace();
						Date dt = new Date();
					...
					if (client_private == null) {
						client_private = "1"; // by default client only run task
												// assign to it.
					}
					client_private = client_private.trim();
					if (client_private.equals("0")) {
						if (client_task) {
							queue_name = priority + "_0_" + System.currentTimeMillis() / 1000 + "#" + queue_name; // higher
																													// priority
						} else {
							queue_name = priority + "_1_" + System.currentTimeMillis() / 1000 + "#" + queue_name; // higher
																													// priority
						}
					} else {
						if (client_task) {
	...
	public HashMap<String, HashMap<String, String>> merge_local_case(Map<String, String> suite_data,
	...
	public static HashMap<String, HashMap<String, String>> merge_link_case(HashMap<?, ?> admin_queue,
			Map<?, ?> task_info) {
		/*
		 * At here, we need to merge the admin queue info and task info together
		 * 1. merge ID 2. merge CaseInfo 3. merge Environment 4. merge
		 * launch_command
		 */
		...
		if (admin_flag) {
			...
			return_info.put("Environment", result_temp3); // merge Environment
															// over
			admin_flag = false;
		}
		...
		if (cmd != null && !cmd.isEmpty()) {
			...
			return_info.put("LaunchCommand", result_temp5); // merge CaseInfo
															// over;
		}
		// System.out.println("In rabbitMQ.merge_link_case the return_info
		// is:"+return_info.toString());
		return return_info;
	}

	public static ArrayList<String> case_prepare(HashMap<String, HashMap<String, String>> case_info) {
		/*
		 * 1: download case 2: download script 3: prepare environment
		 */
		...
		if (caseInfo != null) {
			...
			try {
				...
				String localeString = dt.toString();
				file_action
						.print_out_to_file_to_gui("In case_prepare,\n" + localeString + "\nCan not run system command");
			}
			...
			if (repository != null && repository != "null") {
				...
				// case_dir =
				// "result/"+IDs.get("project")+"/"+IDs.get("run")+"/"+suite_directory+"/"+case_name;
				// // at here, we need trim like action
				...
				// case_dir = "result" + separate + IDs.get("project") +
				// separate + IDs.get("run") + separate + suite_directory +
				// separate + case_name;
				...
				// String cmd = "svn export "+case_url+" "+case_dir +"
				// --username=public --password=lattice";
				...
				ArrayList<String> cmd_array = GetExportCMD(case_url, user_name, pass_word,
						design_path.getPath().replace("\\", "/"));
				try {
					...
					// yu's script ArrayList<?> svn_lines =
					// execute_system_cmd.run(cmd); // at here, we need special
					// process
					// export scripts
					...
					if (script_address != null && script_address != "") {
						...
						svn_script = execute_system_cmd.run("svn export " + script_address + " " + case_dir + "/"
								+ script_name + " --username=public --force --password=lattice --no-auth-cache");
						// file_action.append_file("log/debug.txt", "svn export
						// "+script_address + " "+case_dir+"/"+script_name+"
						// --username=public --force --password=lattice
						// --no-auth-cache " +"\n");
					} else
		...
		if (return_string == null) {
			...
			// String [] init_key = {"diamond", "modelsim", "questasim",
			// "riviera", "icecube"};
				...
				while (request_key.hasNext()) {
					...
					for (int i = 0; i < init_key.length; i++) {
						if (temp_key.equals(init_key[i])) {
							try {
								temp_key_value = ConfigurationFile.getProfileString(defaultConfigure, temp_key,
										temp_key_value, "default");
								if (temp_key_value.equals("default"))
									temp_key_value = ConfigurationFile.getProfileString(defaultConfigure, "soft",
											temp_key_value, "default");
								if (temp_key_value.equals("default"))
									temp_key_value = " ";
							} catch (IOException e) {
	...
	private static ArrayList<String> GetExportCMD(String case_url, String user_name, String pass_word,
			String case_dir) {
		/*
			...
			if (host_src.equalsIgnoreCase("http")) {
				...
				if (host_run.startsWith("windows")) {
					if (url_array[1].startsWith("/")) {
						...
						cmd_array.add("conf\\pscp -r -p -l " + user_name + " -pw " + pass_word + " " + case_url + " "
								+ case_dir);
					} else {
					...
					// D27639:M:/test.txt ./
					cmd_array.add("conf/sshpass/sshpass -p \"" + pass_word + "\" scp -r -p " + user_name + "@"
							+ case_url + " " + case_dir);
				}
			}
		} else {
	...
	public static String getJobDir(String cmd) {
		// String cmd="python sadfsd.py --top-dir=adfsa --job-dir=sfeasfwfs
		// --job-dir=222 d";
	...
	private Boolean local_case_sorting(HashMap<String, HashMap<String, String>> case_data) {
		...
		while (software_it.hasNext()) {
			...
			try {
				client_software_path = ConfigurationFile.getProfileString(defaultConfigure, software_key,
						software_value, "default");
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
		...
		if (mismatch_list.size() > 0) {
			...
			file_action.append_file(publicData.local_skip,
					rpt_string + ". Mismatch:" + mismatch_list.toString() + publicData.line_separator);
			return false;
		} else {
			return true;
		}
	...
	public void local_run_block(Map<String, String> suite_data, Map<String, Map<String, String>> all_case_data) {
		...
		while (case_iterator.hasNext()) {
			...
			// remove unmatch case (case doesn't match the local client
			// requirements)
	...
	public void link_run_block() {
		/*
		 * at here, we need to: 1. read queue_list and get one item <done> 2.
		 * read the item queue and get one case <done> 3. parse case 4. merge
		 * caseinfo, adminqueue info, 5. prepare case(need to download? and so
		 * on) 6. run case 7. Update poolinstance
		 */
		...
		if (!queue_name.isEmpty()) {
			...
			try {
				...
				if (TopRun.dug_model) {
					...
					String localeString = dt.toString();
					file_action
							.print_out_to_file_to_gui("In task_block,\n" + localeString + "\nCan not read_task_server");
				}
			} finally {
			...
			if (task != null && !task.isEmpty() && task != "") {
				...
				HashMap<String, HashMap<String, String>> merge_info = merge_link_case(queue_list.get(queue_name),
						task_info);
	...
	public void run_task_case(HashMap<String, HashMap<String, String>> task_data) {
		...
		if (prepare_string != "1") {
			...
			if (cmd_map == null || !cmd_map.containsKey("cmd")) {
				System.out.println(">>>Warning: Prj" + project_id + "/Run" + run_id + "/T" + case_id
						+ ": No Launch CMD, Skipped.");
				return;
			}
			String cmd_ori = (String) cmd_map.get("cmd");
			String cmd_run = new String();
			if (script_name.equals("")) {
				Pattern patt = Pattern.compile("(?:^|\\s)(\\S*\\.(?:pl|py|rb|jar|class|bat|exe))",
			...
			String[] cmd_list;
			// Callable<?> c = new run_case(cmd_run + "
			// --design="+prepare_case_dir,prepare_string.split(";"));
			...
			this.pool_instance.addTask(c,
					prepare_case_dir + ":" + case_id + ":" + suite_id + ":" + run_id + ":" + project_id + ":" + job_dir,
					timeout, case_style, init_case_name, prepare_case_dir);
	...
	public void monitor_run() {
		...
		while (!stopRequested) {
			...
			try {
				max_process = Integer
						.parseInt(ConfigurationFile.getProfileString(defaultConfigure, "machine", "max_procs", "0"));
			} catch (Exception e) {
				max_process = 0;
			}
			...
			if (used_size < max_process) {
				/*
				 * at here, we need to: 1. read queue_list and get one item 2.
				 * read the item queue and get one case 3. Update poolinstance
				 * 4. parse case 5. run case
				 */
				try {
					if (TopRun.local_model) {
