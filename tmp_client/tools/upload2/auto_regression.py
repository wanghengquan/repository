...
class AutoRegression(object):
    ...
    def dump_options(self):
        ...
        log4u.say_it("  >> dump options to shelve file ...")
        self.dump_to_shelve_file()
    ...
    def dump_to_shelve_file(self):
        for foo in (self.radiant_options_shelve_file, self.diamond_options_shelve_file):
        ...
        for design_path in self.design_paths:
            ...
            options = tools.get_string_options(u"{}".format(res))
            ...
            if "radiant" in base_dict.get("test_software", "") or software_dict.get("radiant"):
                shelve_file = self.radiant_options_shelve_file
            else:
                shelve_file = self.diamond_options_shelve_file
            shelve_ob = shelve.open(shelve_file, writeback=True)
            shelve_ob[design_path] = options
            log4u.say_it(options, "", show=self.debug)
            shelve_ob.close()

    def generate_excel_file(self):
        log4u.say_it("  Generate Suite Excel file ...")
        self.generate_suite_excel_file("diamond", self.diamond_options_shelve_file, self.diamond_suite_excel_file)
        self.generate_suite_excel_file("radiant", self.radiant_options_shelve_file, self.radiant_suite_excel_file)
