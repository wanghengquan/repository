...
class CreateOSCoverageSuite:
    ...
    def dump_to_file(self, suite, ini_dict, cmd_section):
        ...
        has_tot = 0
        for key in list(cmd_section.keys()):
            if key != "all":  # omit all if has _TOT
                has_tot = 1
                break
        else:
            for key, select_cases in list(new_ini_dict.items()):
                partial_path, public_opts = key
        ...
        if has_tot:
            for key, value in list(cmd_section.items()):
