...
from xOS import not_exists, wrap_copy_file, get_fname_ext,get_unique_file
...
class ScanAlteraFmax:
    ...
    def scan_report(self, rpt_file):
        ...
        sdc_files = get_unique_file(sdc_file)
        if sdc_files:
            sdc_file = sdc_files
...
class ScanTimeMem:
    def __init__(self):
        ...
        self.fit_data = {}
        for key in list(self.patterns_fit.keys()):
            key = key.split()[1]
    ...
    def set_fit_data(self):
        for key in list(self.fit_data.keys()):
            self.fit_data[key] = 'NA'
    def reset(self):
        self.set_srr_data()
        self.set_fit_data()
