...
class write_excel:
    ...
    def write_note(self):
        ...
        for cmp in self.cmps:
            ...
            for key in list(cmp.note.keys()):
                if key in list(dict_return.keys()):
    ...
    def write_body(self,row):
        ...
        for num,cmp in enumerate(self.cmps):
            ...
            all_case = all_case + list(cmp.csv_dic.keys())
    ...
    def write_note_Note(self,row):
        ...
        row = row+2
        all_cases =[]
        all_cases += list(self.src_dict_note_dict.keys())
        for item in self.cmp_dict_note_dict:
            all_cases +=list(item.keys())
...
def write_total_summay(statistic='',line='',summary={}): #only for QoR
        ...
        for sheet_f in list(summary.keys()):
            try:
        ...
        for p_id,item in enumerate(priority):
            ...
            for s in sheets:
                sheet_percent_titles = sheet_percent_titles + list((summary[s][1]).keys())
            ...
            sheet_row_titles = []
            for s in sheets:
                sheet_row_titles = sheet_row_titles + list((summary[s][0]).keys())
            ...
            for row_title in sheet_row_titles:
                ...
                for s in sheets:
                    ...
                    if not s_abs:
                        ...
                        for col_key in list(s_abs.keys()):
                            abs.setdefault(col_key,[]).append(s_abs[col_key])
                for col_key in list(abs.keys()):
        ...
        for sheet_f in list(summary.keys()):
            ...
            #print sheet_f_first
            for sheet_f_first_key in list(sheet_f_first.keys()):
                if sheet_f_first_key in list(all_data.keys()):
                    pass
                else:
        ...
        row_title_keys = list(all_data.keys())
