...
class ScanRunPBLog(ScanBasic):
    ...
    def create_patterns(self):
        self.patterns = OrderedDict()
        # Lattice Radiant Software  Version 1.0.0.348.0
        self.patterns["Version"] = re.compile("Lattice\s+Radiant\s+Software\s+Version\s+(.+)")
...
class ScanParFile(ScanBasic):
    ...
    def create_patterns(self):
        ...
        self.patterns["ParPeakMem"] = re.compile("after\s+PAR.+maximum\s+.+\s+memory\s+([\d\.]+)")
...
class ScanRadiant:
    ...
    def process(self, arg_list=list()):
        ...
        new_data_dict = dict(list(zip(re.split(",", list2str(title)), re.split(",", list2str(data)))))
...
if __name__ == "__main__":
    t = ScanRadiant("Radiant_report.csv")
    t.process()
