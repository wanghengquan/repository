...
class scan_general_case:
    ...
    def scan_set(self):
        ...
        if self.report_file == 'report.csv':
            self.report_file = os.path.basename(self.job_dir)+'_old.csv'
    ...
    def for_bqs_data_f(self,all_time_data):
        #jason add dir_data for one dir scan data record
        #print '123##################################################'
        ...
        #No BQS need jason comment it at 2017/03/30 
    ...
    #jason added for qor data merge
    def get_total_dir_data(self,all_time_data):
        dir_data = dict()
        for i_d in all_time_data:
            dir_data[i_d] = all_time_data.get(i_d,'NA')
        for id_f,i_d in enumerate(self.scan_mrp.get_title()):
            try:
                dir_data[i_d] = self.scan_mrp.get_data()[id_f]
            except:
                pass
        for id_f,i_d in enumerate(self.scan_twr.get_title()):
            if i_d == 'Pass_num':
                continue
            try:
                dir_data[i_d] = self.scan_twr.get_data()[id_f]
            except:
                continue
        return dir_data
    ...
    def qor_merge_console_report(self, all_scan_data):
        #seed or sweeping point
        console_rpt_data = dict()
        best_fmax = 0        
        for point in all_scan_data:
            point_data = all_scan_data[point]
            if "fmax" not in point_data:
                console_rpt_data = point_data
            else :
                current_fmax = point_data["fmax"]
                if string.atof(current_fmax) > best_fmax:
                    best_fmax = string.atof(current_fmax)
                    console_rpt_data = point_data
        print("===TMP Detail===")
        for key in list(console_rpt_data.keys()):
            print("\t<%s>" % key + console_rpt_data[key] + "</%s>" % key) 
        print("===TMP end===")               
    ...
    def scan_qor_report(self):
        ...
        for design in self.all_designs:
            ...
            for dir in os.listdir(design_path): # dir:Target_Fmax_is_060MHz, get the srr file 
                ...
                if (not re.search("Target", dir)) and os.path.isdir(dir_scan) and not srr_file:
                    ...
                    if not srr_file:
                        srr_file = get_unique_file(dir_scan+ "/synthesis.log")
            find_target_dir = 0
            #jason added for merge reports into one final report.
            all_scan_data = dict();
                     ...
                     if twr_file:
                         ...
                         if  self.force_NAN:
                             if all_time_data['Complete'] == 'NA' or all_time_data['Par_Done'] == 'NA':
                                 ...
                                 data_list = self.scan_mrp.get_data() + \
                                   self.scan_twr.get_data() + data_list
                         #all_scan_data[dir] = all_time_data
                         #self.for_bqs_data_f(all_time_data)
                         all_scan_data[dir] = self.get_total_dir_data(all_time_data)
            ...
            if find_target_dir == 0:
                self.write_NA_all_report(self.design)
        os.chdir(root_dir)
        self.qor_merge_console_report(all_scan_data)
