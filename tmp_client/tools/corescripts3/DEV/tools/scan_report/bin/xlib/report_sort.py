...
def sort_csv(csv_file,col_key='fmax',fail_log='',add_average='',list_order=True,name_list='',default_case_list=""):
    ...
    for id1,item in enumerate(ori_dict):
        ...
        if sw2 == 'None' or sw2 == '' or sw2 == '-' or sw2 == '_':
            ...
            note_case = item
        if id1 == 0:
            for group,cases in list(group_cases.items()):
                if design in cases:
    ...
    if find_it == 1 and list_order:
        ...
        note_case = item
        cases_list = list(design_line_dic.keys())
        cases_list.sort()
    ...
    #------------------------------END---------------------------------------------------#
    fmax_average = []
    for key,value in list(design_line_dic.items()):
        pass_points = 0
        for result in value:
    ...
    cases = list(design_line_dic.keys())
    cases.sort()
...
def get_best_all_clk(file_sorted):
    ...
    dict_writer.writerow( dict( list(zip(titles,titles)) ) )
