...
def print_str(s, file_handle=sys.stdout):
    ...
    if sys.__stdout__.encoding:
        print(s.encode(sys.__stdout__.encoding, 'replace'), file=file_handle)
    else:
        print(s.encode("utf-8"), file=file_handle)
...
NEUTRAL_COLOR = "#DCDCDC"  # gainsboro
PASS_COLOR = "#f0fff0"     # honeydew
FAIL_COLOR = "#FFB6C1"     # lightpink
ERROR_COLOR = "#FA8072"    # salmon
LOG_COLOR = "#DAA520"      # goldenrod
WARNING_COLOR = "#FFA500"  # orange
FATAL_COLOR = "#F08080"    # lightcoral
CASE_COLOR = "#90ee90"     # lightgreen
...
INDEX_HTML_END = "</table></body></html>\n"

SUBPAGE_HTML_END = "</body></html>\n"

INDEX_ITEM = """<tr valign="%(valign)s" bgcolor="%(color)s">\
...
SUMMARY_ITEM = """<tr valign="%(valign)s" bgcolor="%(color)s">\
<td>%(name)s</td><td align="right">%(value)s</td><td>%(extra)s</td>\
</tr>\n"""

CASE_ITEM = """<tr valign="%%(valign)s" bgcolor="%s"><td>\
<b>%%(name)s</b></td><td colspan="4">%%(start)s</td></tr>\n""" % (
        CASE_COLOR)

VERIFICATION_ITEM = """<tr valign="%%(valign)s" bgcolor="%s">\
<td>%%(name)s</td><td colspan="4">%%(filename_and_line)s</td>\
</tr>\n""" % NEUTRAL_COLOR

SAXPARSEEXCEPTION_ITEM = """<tr valign="%%(valign)s" bgcolor="%s">\
...
RESULT_ITEM = """<tr valign="%%(valign)s" bgcolor="%s"><td></td>\
...
class SquishReportHandler(xml.sax.handler.ContentHandler, xml.sax.handler.ErrorHandler):
    ...
    def startElement(self, name, attributes):
        if name == "SquishReport":
            version = attributes.get("version").split(".")
        ...
        if name == "test":
            ...
            if self.result_type == "PASS":
                self.suite_passes += 1
            elif self.result_type == "FAIL":
                self.suite_fails += 1
            elif self.result_type == "FATAL":
                self.suite_fatals += 1
            elif self.result_type == "ERROR":
                self.suite_errors += 1
            elif self.result_type in ("XPASS", "UPASS"):
                self.suite_unexpected_passes += 1
            elif self.result_type == "XFAIL":
    ...
    def endElement(self, name):
        if name == "SquishReport":
            ...
            color = FAIL_COLOR
            if self.result_type in ("PASS", "XFAIL"):
                color = PASS_COLOR
            elif self.result_type == "ERROR":
                color = ERROR_COLOR
            elif self.result_type == "LOG":
                color = LOG_COLOR
...
def escape_and_handle_image(description, preserve, absVpFile=None, widgetName=None, squishDir=None, subFilesDir=None, tempDir=None):
    match = re.search(r"""saved\s+as\s+['"](?P<image>[^'"]+)['"]""",
