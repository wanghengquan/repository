...
def _add_quotes(raw_string):
    """
    Convert system arguments to right format
    """
    if re.search("\s", raw_string):
        raw_string = re.sub("([^=]+)=(.+)", '\g<1>="\g<2>"', raw_string)
    return raw_string

def add_cmd_line_history(log_file):
    new_line = sys.executable + " " + " ".join(map(_add_quotes, sys.argv))
...
def get_xrange(a_range):
    ...
    for item in range(a_range[0], a_range[1]+1, a_range[2]):
        t_list.append(item)
...
p_license_error = [re.compile(r"check\s+your\s+license\s+setup\s+to\s+ensure"),
...
special_error_p = re.compile("^@E:", re.I)
p_error_msg = [re.compile("^\s*error", re.I),
               re.compile("^wrong\s+", re.I),
               re.compile("fatal\s+error", re.I),
...
def run_command(cmd, log_file, time_file):
    ...
    while True:
        ...
        etime.stop()
        append_file(log_file, ["=" * 30, cmd, etime] + text, append=True)
        append_file(time_file, [cmd, "rem %s" % etime], append=True)
        if sts:
            t = "Status: Failed."
        else:
            t = "Status: Pass."
        ...
        if have_license_error:
            ...
            if abnormal_exit(log_file):
                ...
                break
    if sts:
        # use "@E:" as the first error message
        error_patterns = [[special_error_p], p_error_msg]
        for ps in error_patterns:
...
def play_announce(src_design, job_design, comments=list()):
    ...
    """
    lines = list()
    lines.append("* Play Time: %s" % get_shanghai_time())
    lines.append("*---------------------------------------------")
    lines.append("")
...
def stop_announce(play_time):
    ...
    say_it("* Stop Time: %s" % get_shanghai_time())
    say_it("*")
