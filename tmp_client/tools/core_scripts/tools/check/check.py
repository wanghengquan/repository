...
class TA:
    ...
    def check_it2(self, design_path):
        ...
        for conf_file in conf_files:
            ...
            try:
                ...
                pass
            #jason add for bypass the cr_note    
            for item ,value in conf_inf.items():
                if item !="cr_status":
                    continue
                if "0" == value.strip():
                    self.cr_status_flag = False
                else:
                    self.cr_status_flag = True
            #only cr_status present and value is 1 then cr_note will work
            for item ,value in conf_inf.items():
                if item =="area":
                    self.area = value
                elif item == "type":
                    self.type = value
                elif item == "cr_note" and self.cr_status_flag:
                    self.Comments = value
                    if value.strip():
    ...
    def check_lines(self):
        ...
        check_line_compile = [re.sub("\s+", " ", item) for item in check_lines]
        line = file_hand.readline()
        line_time = 0
        while line:
            line = re.sub("\s+", " ", line)
    ...
    def check_lse(self):
        '''
            ...
            criteria 2: must find string "CPU time for LSE flow" at the bottom of report file
        ...
        #print file_name
        result_single = "Fail"
        check_lines = 'CPU time for LSE flow'
        line = file_hand.readline()
        while line:
            line = line.strip()
    ...
    def clk_reference_flow(self, check_file = '',check_pattern = '',init_result = '',allowance = '', clk_name = ''):
        ...
        check_pattern = re.compile(check_pattern, re.I)
        check_result = ''
        ...
        if check_result == 0:
            ...
            if init_result == 0:
                ...
                check_allowance = (check_result-init_result)/init_result*100
                print 'run 2150'
                print check_allowance
                print allowance/100
                #jason added 20161012
                if check_allowance < (allowance * -1):
                    print_log( 'check_result error '+str(check_result) )
                    temp_clk_record[4] = str(check_result)
                    temp_clk_record[5] = str(check_allowance)+'%'
                    temp_clk_record[6] = 'FAIL'
                    self.lines_reference_2.append(temp_clk_record)
                else:
                    temp_clk_record[4] = str(check_result)
    ...
    def check_diamond_flow(self):
        ...
        flow_check_dict = {
                             ...
                             'bitstream':   {'sub_path':'_scratch/*/*.bit;_scratch/*/*.rbt', 'check_string':'FILE_EXISTS'},
                             'jedec'    :   {'sub_path':'_scratch/*/*.jed', 'check_string':'FILE_EXISTS'},
                             'download' :   {'sub_path':'_scratch/*/*.bit;_scratch/*/*.rbt;_scratch/*/*.jed;_scratch/*/*.bin;_scratch/*/*.hex;_scratch/*/*.nvcm', 'check_string':'FILE_EXISTS'},
    ...
    def diamond_file_check(self, flow_name, flow_check_dict):
        ...
        path_lst   = sub_path.split(';')
        work_lst = []
        file_list = []
        for path in path_lst:
            work_lst.append(os.path.join(self.design_path, path))
        for path in work_lst:
