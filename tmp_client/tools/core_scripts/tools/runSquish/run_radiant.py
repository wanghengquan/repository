...
%(squishrunner)s --config setAUTTimeout 60
%(squishrunner)s --config setResponseTimeout 60
%(squishrunner)s --testcase %(testcase)s --lang %(language)s --wrapper %(wrapper)s --objectmap %(objectmap)s --cwd %(cwd)s
%(squishserver)s --stop
'''
...
class RunSquishCase:
    ...
    def run_batch_file(self):
        ...
        if not self.aut:
            if self.on_win:
                name_of_aut = "pnmain.exe"
            else:
                name_of_aut = "radiant"
        else:
            if os.path.isfile(self.aut):
                name_of_aut = os.path.basename(self.aut)
                batch_kwargs["path_to_aut"] = os.path.dirname(self.aut)
            else:
                name_of_aut = xTools.get_fname(self.aut)
                if self.on_win:
                    name_of_aut += ".exe"

        if self.on_win:
            batch_kwargs["set_path"] = r'set PATH=%%PATH%%;%s;%s\bin' % (os.path.join(self.radiant, "bin", self.nt_lin), self.squish)
