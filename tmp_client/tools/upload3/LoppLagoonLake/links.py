...
def get_tr_api(username="", password=""):
    ...
    api = testrail.APIClient("http://lsh-tmp/testrail/")
    api.user = username
...
class TestRail(object):
    ...
    def get_count_fields(self):
        ...
        self.count_fields = list(filter(f, [item.get("Field") for item in res]))
    ...
    def get_pass_total(self, plan_id):
        ...
        total_count = sum([plan_count_data.get(key) for key in list(plan_count_data.keys())])
