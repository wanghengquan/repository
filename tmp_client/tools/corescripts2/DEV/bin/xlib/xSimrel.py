    ...
    with open(pad_file) as pad_ob:
        for line in pad_ob:
            ...
            if not titles:
                ...
                if not line:
                    break
                t_dict = dict(zip(titles, line_list))
...
def get_avc_bin_code(lst_data, ports_in, ports_bidi, ports_vref, ports_plus, ports_minus,
    ...
    for (_ports, is_minus) in ((ports_plus, False), (ports_minus, True)):
        for show_name, inner_name in _ports.items():
...
def generate_avc_file(avc_file, lst_file, pad_file, sim_vendor_name, simrel_options):
    ...
    for bar in (ports_plus, ports_minus):
        second_line_list.extend(bar.keys())
        for cat, raw_cat in bar.items():
    ...
    third_line.append(";")
    print >> avc_ob, " ".join(second_line)
    print >> avc_ob, " ".join(third_line)
    for item in lst_dict:
        ...
        print >> avc_ob, "r1 name  %s;" % avc_bin_code
    avc_ob.close()
# --------------------------
...
def run_sim_ncv(general_options, simrel_dir, rbt_avc_folder, simrel_options, use_original_ctl):
    ...
    for _file in src_files:
        ...
        if os.path.isfile(a):
            print >> temp_sh, "rm %s" % _file
        print >> temp_sh, "cp -f %s %s" % (a, _file)
    for _file in dst_files[:-1]:
        ...
        if os.path.isfile(_file):
            print >> temp_sh, "rm %s" % _file
    if use_original_ctl:
        print >> temp_sh, "cp {} fc.ctl".format(original_fc_ctl)
    if os.path.isfile("rbt_to_sspi_init_bus.pl"):
        print >> temp_sh, "perl rbt_to_sspi_init_bus.pl fc.rbt"
    print >> temp_sh, "# use original fc.tcl file: {}".format(use_original_ctl)
    temp_sh.close()
...
def get_sentry_simrel_path(size_in_device):
    ...
    for _num, _size in sentry_size_dict.items():
