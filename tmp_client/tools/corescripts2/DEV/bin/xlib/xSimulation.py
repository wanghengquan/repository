...
class RunSimulationFlow:
    ...
    def flatten_options(self):
        ...
        if sts:
            return 1
        if self.is_ng_flow:
            radiant_sim_map_dict = raw_lib_dict.get("radiant_map_sim_lib", dict())
            for k, v in radiant_sim_map_dict.items():
        ...
        if res:
            self.do_args["resolution"] = "-t %s" % res
        else:
                ...
                if start_source:
                    if self.p_src_end.search(line):
                        ...
                        if not v_v_line:
                            ...
                            # return 1
                        if self.sim_vendor_name == "Riviera":
                            v_v_line = map(add_dbg_for_vlog, v_v_line)
            ...
            for item in src_files:
                ...
                if fext in (".v", ".vo", ".sv", ".vm"):
                    if p_for_pmi.search(item):
                        ...
                        v_v_lines.append("vlog %s %s" % (x, item))
