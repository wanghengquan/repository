...
def get_valid_crs(cr_note):
    valid_crs = []
    cr_note = cr_note.split(',')
    verified = ['verified', 'closed']
    cr_grep = re.compile(r'(^SOF-\d+|^DNG-\d+)', re.I)
    url = 'http://ldc-jira01:8080/rest/api/2/search'
    headers = {'Content-Type': 'application/json'}
    for cr in cr_note:
        cr = cr.strip()
        if not cr:
            continue
        search_cr = cr_grep.search(cr)
        if not search_cr:
            if cr == 'NA':
                continue
            log(cr + ' is history cr_note usage! need update!', logging.WARNING)
            continue
        cr = search_cr.group(0)
        data = json.dumps({'jql': 'key=' + cr, 'startAt': 0, 'maxResults': 100000})

        """ use urllib2 instead of requests"""
        for i in range(Default.MAX_TRIES * 2):  # double try times
            auth = Default.AUTHS[divmod(i, len(Default.AUTHS))[1]]
            try:
                req = urllib2.Request(url, data=data, headers=headers)
                base64string = base64.b64encode('%s:%s' % (auth[0], auth[1]))
                req.add_header("Authorization", "Basic %s" % base64string)
                response = urllib2.urlopen(req)
                r = json.loads(response.read())['issues'][0]
                issuetype = r['fields']['issuetype']['name']
                priority = r['fields']['priority']['name']
                status = r['fields']['status']['name']
                components = r['fields']['components'][0]['name']
                log("Begin to check cr note: " + cr, logging.INFO)
                log("Issue Type: " + issuetype, logging.INFO)
                log("Priority: " + priority, logging.INFO)
                log("Status: " + status, logging.INFO)
                log("Component: " + components, logging.INFO)
                if status.lower() not in verified:
                    log(cr + ' is a valid cr_note!', logging.INFO)
                    valid_crs.append(cr)
                else:
                    log('cr_note:    ' + cr + (20 - len(cr)) * ' ' + ' has been verified!')
                break
            except Exception as e:
                if 'Error 400' in str(e):
                    log('cr_note:    ' + cr + (20 - len(cr)) * ' ' + ' this cr is not exist, please check it.')
                    break
                elif i < Default.MAX_TRIES - 1:
                    time.sleep(random.random() + 0.5)
                    continue
                elif 'Error 401' in str(e):
                    log('cr_note:    ' + cr + (20 - len(cr)) * ' ' + ' jira authorize error with ' + auth[0])
                else:
                    log(str(e), logging.ERROR)
                    log('cr_note:    ' + cr + (20 - len(cr)) * ' ' + ' unable to check this cr with ' + auth[0])
            time.sleep(5)

        """ use request instead of urllib2
        ...
        if r:
            ...
            if status.lower() not in verified:
                ...
                log('cr_note has been verified!', logging.ERROR)
        else:
            log('cr_note ' + cr + ' is invalid!', logging.ERROR)
        """
    return valid_crs
...
def get_designs(args):
    ...
    if not os.path.isdir(top):
        raise (ConfigIssue('top dir ' + top + ' not exist!'))
    design = args.design
    designs = OrderedDict()
    set_log(top)
    if design:
        ...
        if not os.path.isdir(design):
            raise(ConfigIssue('design ' + design + ' not exist!'))
...
def start_check_flow(designs, args):
    ...
    for design, conf_files in designs.items():
        if len(conf_files) == 0:
            continue
...
def show_check_data_detail(check_data_designs):
    for design, check_data_config in check_data_designs.items():
        log('\n+- design : ' + design)
        for conf_file, check_data_section in check_data_config.items():
            ...
            log(start_time)
            for key_case, value_case in case_info.items():
                log('+---- ' + key_case + ' : ' + value_case)
            for key_sec, value_sec in section_info.items():
                log('+\n+-------- ' + str(key_sec))
                for key, value in value_sec.items():
                    log('+-------------------- ' + key + ': ' + str(value))
            log(end_time)
...
def get_status_from_check_data(check_data_designs):
    ...
    for design, check_data_config in check_data_designs.items():
        for conf_file, check_data_section in check_data_config.items():
            ...
            for method, sections in section_info.items():
...
def get_section_result(check_result):
    ...
    for name, results in check_result.items():
        if results[0]:
...
def get_final_status(check_data_designs, args):
    ...
    for design, check_data_config in check_data_designs.items():
        ...
        for _, check_data_section in check_data_config.items():
            ...
            for section_method, section_result in section_info.items():
            ...
            for name, final_sec_ret in final_section_result.items():
...
def create_check_report(check_data_design, args):
    ...
    for design_config, details in status.items():
