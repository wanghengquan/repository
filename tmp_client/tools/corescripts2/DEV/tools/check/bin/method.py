...
class Method(object):
    ...
    def do(self, queue, env):
        ...
        while not queue.empty():
            ...
            for key, _ in section_info.items():
    ...
    def pre_handle(self, task, section):
        if 'show' in section.keys() and section['show'] != '0':
            self.show = True
...
class PollMethods:
    ...
    def do(self):
        ...
        for task_method, task_queue in self.env.conf_tasks.items():
...
class CheckSimulationFlow(Method):
    def __init__(self):
        ...
        self.func_name = "check_simulation_flow"
    ...
    def handle(self, task, section):
        ...
        for chk_name in will_check_flow_list:
            ...
            if chk_name not in self.valid_choices:
                self.log_error("Warning. Unknown name {} for {}".format(chk_name, self.func_name))
                continue
...
class CheckSdfSimulation(Method):
    def __init__(self):
        ...
        self.func_name = "check_sdf_simflow"
    ...
    def handle(self, task, section):
        ...
        #
        pass_list = filter(lambda x: x.startswith("passkey"), section.keys())
        if not pass_list:
            if is_modelsim:
        ...
        for one, two in check_pass_dict.items():
            start_index = -1
            if is_modelsim:
            ...
            if not ret:
                ...
                return
        #
        fail_list = filter(lambda x: x.startswith("failkey"), section.keys())
        if fail_list:
            ...
            if is_modelsim:
                ...
                check_fail_dict = dict(failkey1='SDF: 0 SDF entries loaded',
                                       failkey2='SDF: Error')
        for one, two in check_fail_dict.items():
...
class CheckRadiantFlow(Method):
    def __init__(self):
        Method.__init__(self)
        self.func_name = 'check_radiant_flow'
        self.basic_flow = {
    ...
    def handle(self, task, section):
        self.ret = Default.PASS
        support_check_flow = self.basic_flow.keys() + self.logic_flow.keys()
        ...
        if 'debug' in section and section['debug']:
            ...
            for k, v in self.basic_result.items():
...
class CheckDiamondFlow(CheckRadiantFlow):
    def __init__(self):
        ...
        self.func_name = 'check_diamond_flow'
        self.basic_flow = {
            'synp': {'sub_path': '_scratch/*/*.srr', 'check_string': 'Mapper successful!'},
...
class SimCheckBlock(CheckBlock):
    def __init__(self):
        ...
        self.func_name = 'sim_check_block'
        self.sim_type = True
...
class CheckMultiline(Method):
    def __init__(self):
        Method.__init__(self)
        self.func_name = 'check_multiline'
...
class CheckComparepar(Method):
    def __init__(self):
        Method.__init__(self)
        self.func_name = 'check_compare_par'
...
class CheckFile(Method):
    def __init__(self):
        Method.__init__(self)
        self.func_name = 'check_file'

    def handle(self, task, section):
        filename = os.path.abspath(section['file'])
        reverse = section['reverse'] if 'reverse' in section.keys() else ''
        files = glob.glob(filename)
        if len(files) == 0:
            self.ret = Default.FAIL
        if reverse == '1':
...
class CheckClkReference(Method):
    def __init__(self):
        Method.__init__(self)
        self.func_name = 'check_clk_reference'
...
class CheckLutReference(CheckClkReference):
    def __init__(self):
        ...
        self.func_name = 'check_lut_reference'
...
class CheckSim(Method):
    ...
    def handle(self, task, section):
        if 'file' in section.keys():
            self.filename = section['file']
        if 'language' in section.keys():
            self.language = section['language'].lower()
        if 'passkey' in section.keys():
            self.passkey = section['passkey']
        if 'failkey' in section.keys():
            self.failkey = section['failkey']

        if self.language == 'vhdl' and self.func_name != 'check_sim_rtl':
        ...
        self.log_info(self.func_name + ' pass!')


class CheckRtlSim(CheckSim):
    def __init__(self):
        CheckSim.__init__(self)
        self.func_name = 'check_sim_rtl'
        self.path = './_scratch/sim_rtl'
        self.filename = 'outlog.log'
        self.language = 'verilog'
        self.passkey = 'PASS'
...
class CheckClk(Method):
    ...
    def handle(self, task, section):
        ...
        try:
            ...
            raise ConfigIssue('wrong option in ' + self.func_name)

        if self.func_name == 'check_clks_lse':
...
class CheckSta(Method):
    ...
    def handle(self, task, section):
        ...
        try:
            if 'lse' in self.func_name:
                filename = filename[1]
            else:
                filename = filename[0]
        except Exception:
            raise ConfigIssue(self.func_name + ' fail, twr file not exist!')
        ...
        for sta_check in section :
            if re.match(r'^check\d$', sta_check):
                ...
                if len(s) == 2:
                    ...
                    if not times:
                        raise ConfigIssue('wrong times= option in ' + self.func_name)
                    else:
                        times = times.group(1)
                else:
                    raise ConfigIssue(self.func_name + ' incorrect!')
                if times is None:
    ...
    def get_columns(self):
        ...
        with open(self.avc, 'r') as f:
            ...
            if self.signals:
                ...
                signals.reverse()
                for sig, shift in self.signals.items():
