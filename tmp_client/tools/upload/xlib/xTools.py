...
LOCK_FILE = "lock_for_safety_flow"
...
def get_conf_options(conf_files, key_lower=True):
    ...
    p_multi = re.compile("%s(.+)%s" % (start_mark, end_mark))
    conf_options = dict()
    if key_lower:
    ...
    try:
        ...
        for section in conf_parser.sections():
            ...
            for option in conf_parser.options(section):
                ...
                if value_list:
                    value_list = [item.rstrip() for item in value_list]
                    if option in ("description", "announcement"):
                        value = os.linesep.join(value_list)
                    else:
                        value = value_list[:]
...
def run_command(cmd, log_file, time_file, try_one_time=0):
    """
...
def sleep_and_run(my_sched, func, sleep_time, func_options):
    """run the flow after sleep_time

    :param func: function name
    :param sleep_time:
    :param func_options: func options is A SINGLE list.
    :return:
    """
    my_sched.enter(sleep_time, 0, sleep_and_run, (my_sched, func, sleep_time, func_options))
    func()

def loop_sleep_run(my_sched, func, sleep_time, func_options=""):
    """loop run with sleep_time
...
def progress_test(sleep_time):
    bar_length=20
    sleep_step = sleep_time / 100.0
    for percent in xrange(0, 101):
        hashes = '.' * int(percent/100.0 * bar_length)
        spaces = ' ' * (bar_length - len(hashes))
        sys.stdout.write("\r  Percent: [%s] %d%%"%(hashes + spaces, percent))
        sys.stdout.flush()
